<template>
  <a-modal :width="modalWidth" title="新增按钮" :confirmLoading="loading" v-model="show"
    ...
    <a-form :form="form">
      <a-row :gutter="24">
        ...
        <a-col :span="24">
          <a-form-item label="按钮权限" :style="{ display:  'block' }"
                       :label-col="{ span: 4 }"
                       :wrapper-col="{ span: 20 }">
  ...
  methods: {
    ...
    handleOk () {
      this.form.validateFields(err => {
        if (!err) {
          this.loading = true
          let buttonFormData = this.form.getFieldsValue()
          buttonFormData.parentTreeId = this.parentTreeId
          addButton(buttonFormData).then(res => {
            if (res.code === 200) {
